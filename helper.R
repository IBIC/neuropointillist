...
#### Read in data sets.
#### Return the design matrix and a data frame with the selected vertices 
readDataSets <- function(args, numberdatasets, mask.vertices) {
    alldat <- c()
    covariates <- c()
    dataset<-1
    dims <- c()
...
#### Write output files. This writes out the output statistic files.
writeOutputFiles <- function(prefix, results, mask) {
...
splitDataSize <- function(size, prefix,mask) {
    dir <- dirname(prefix)
    if (!dir.exists(dir)) {
        dir.create(dir, recursive=TRUE)
    }
...
writeMakefile <- function(prefix, resultnames, modelfile, designmat, makefile) {
    dir <- dirname(prefix)
    if (!dir.exists(dir)) {
        dir.create(dir, recursive=TRUE)
    }
...
writeSGEsubmitscript <- function(prefix, resultnames, modelfile, designmat,masterscript,jobscript,njobs) {
    dir <- dirname(prefix)
    if (!dir.exists(dir)) {
        dir.create(dir, recursive=TRUE)
    }
